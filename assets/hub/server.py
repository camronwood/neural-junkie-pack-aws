#!/usr/bin/env python3
"""AWS pack hub sidecar entrypoint."""
...
class PackHubHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            return
        if path.startswith("/api/aws/"):
            self._handle_aws_post(path, body)
            return
        self._json(404, {"error": "not found"})

    def _handle_aws_get(self, path: str) -> None:
        from routes import aws

        aws.handle_get(self, path, self.settings, self.pack_dir)

    def _handle_aws_post(self, path: str, body: dict) -> None:
        from routes import aws

        aws.handle_post(self, path, body, self.settings, self.pack_dir)
